...
1. Create or reuse a crates.io API token with publish access for the intended graph crates.
...
5. `use-graph-path`
6. `use-weighted-graph`
7. `use-graph-metrics`
8. `use-graph`

The umbrella crate `use-graph` should come last after all focused crates are visible on crates.io.
...
```sh
cargo publish --dry-run --allow-dirty -p use-node
```

Dry-run the umbrella crate only after the focused crates are live on crates.io:

```sh
cargo publish --dry-run --allow-dirty -p use-graph
```
